...
const Timeline = ({ setObserver, callback, props }) => {
    ...
    const someCallback = (engineerType,i) => {
        ...
        console.log(engineerType+'Details'+i)
    ...
    useEffect(() => {
        ...
        props.internList?.filter((intern => intern.engineerType === "data")).map((intern, i) => {
            if(intern.engineerType === "data"){
                if(i === props.internList.filter((intern => intern.engineerType === "data")).length -1){
                    setObserver(props.getRef(intern.id).current, ()=>someCallback("contact"))
        ...
        setObserver(props.getRef("contact1").current, ()=>someCallback("contact",1))
    }, []);
